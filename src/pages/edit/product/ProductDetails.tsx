...
export const ProductDetails = ({ pickedProduct, pickedVariant }: TProductDetailsProps) => {
  return (
    <div className="smd:order-1 3xl:text-5xl order-2 w-full flex flex-col">
      <div className="smd:order-1 order-2 pl-1 pt-2">
        <h1 className="sm:text-[2em] 3xl:text-[1em] smd:mt-0 mt-2 leading-tight text-[1.5em] font-bold text-slate-900 mb-2">
          {pickedProduct.name}
        </h1>
      </div>

      <div className="smd:order-2 order-3 flex items-center space-x-3 pl-1">
        <span className="3xl:text-[1em] text-3xl text-orange-600">
          <span className="font-bold">
            {formatNumberWithCommas(pickedVariant.priceAmountOneSide)}
          </span>
          <span className="3xl:text-[0.8em] text-2xl font-medium ml-1">
      ...
      <div className="smd:order-3 order-4 smd:p-3 p-2 bg-orange-50 border border-orange-100 rounded-lg space-y-2 my-2">
        <div className="3xl:text-[0.5em] text-sm flex items-center justify-between gap-2">
